...
#define level11Path [[NSBundle mainBundle] pathForResource:@"Level11" ofType:@"arch"] 
#define level12Path [[NSBundle mainBundle] pathForResource:@"Level12" ofType:@"arch"] 
#define level13Path [[NSBundle mainBundle] pathForResource:@"Level13" ofType:@"arch"] 
#define level14Path [[NSBundle mainBundle] pathForResource:@"Level15" ofType:@"arch"] 
#define level15Path [[NSBundle mainBundle] pathForResource:@"Level16" ofType:@"arch"] 
#define level16Path [[NSBundle mainBundle] pathForResource:@"Level17" ofType:@"arch"] 
#define level17Path [[NSBundle mainBundle] pathForResource:@"Level18" ofType:@"arch"]
#define level18Path [[NSBundle mainBundle] pathForResource:@"Level14" ofType:@"arch"] 
#define level19Path [[NSBundle mainBundle] pathForResource:@"Level19" ofType:@"arch"] 
#define level20Path [[NSBundle mainBundle] pathForResource:@"Level20" ofType:@"arch"] 

@implementation LevelMenu2
...
-(id)init
{
	...
	if(self != nil)
	{
		...
		locationLevels = [[NSArray alloc]initWithObjects:level11Path,level12Path,level13Path,level14Path,level15Path,level16Path,level17Path,level18Path,level19Path,level20Path,nil];
		...
		int indexNum = [self getLatestLevelIndex]-10;		
		NSIndexPath *pathIndex = [NSIndexPath indexPathForRow:indexNum inSection:0];
...
-(void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	if(point.x > 11 && point.x < 36 && point.y > 359 && point.y < 459)
	{
		canContinue = NO;
		[[SimpleAudioEngine sharedEngine]playEffect:@"Button.caf"];
		[[CCDirector sharedDirector] replaceScene:[LevelMenu3Scene node]];
	}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if([self levelCanBeShown:indexPath.row+10])
	{
		
		GameCard *card = [[[WBManager sharedManager]currentUser]getGameCardForLevel:indexPath.row+10];
		...
		levelName.text = [self getLevelStringForIntValue:indexPath.row+10];
		...
		UIImage *screenshot = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"WSimg%i",indexPath.row+1+10] ofType:@"PNG"]];
...
-(int)getLatestLevelIndex
{
	...
	if(index != 19 && index != -1)return index+1;
	else if(index == 19)return index;
	else return 10;
}
