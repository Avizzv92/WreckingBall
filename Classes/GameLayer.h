...
@interface GameLayer : CCLayer <UIAccelerometerDelegate,GamePopUpDelegate, UIApplicationDelegate, UITextFieldDelegate>
{	
	int gravityForLocation;
	
	//For storing original sound volume
	float oldVolume;
	
	BOOL hasCheated;
	
	cpCCSprite *platFormCoupler;
	
	UITextField *codeField;
	BOOL codeFieldActive;
	
	NSMutableDictionary *settingsDictionary;
	
	AVAudioPlayer *countrysideSound;
	AVAudioPlayer *wildernessSound;
	AVAudioPlayer *citySound;
	...
	//If the water has been touched or not
	BOOL waterTouched;
	//
	BOOL buidlingTouched;
	...
	BOOL directionButtonActive;
	
	bbLocation theWorksiteLocation;
}
...
//Called when a level has failed (by touching something that should not have been touched)
